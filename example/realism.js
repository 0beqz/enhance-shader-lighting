...
const createGlobalDisableIblIradianceUniform = () => {
  if (
    !ShaderChunk.envmap_physical_pars_fragment.includes("iblIrradianceDisabled")
  ) {
    ShaderChunk.envmap_physical_pars_fragment =
      ShaderChunk.envmap_physical_pars_fragment.replace(
  ...
  ShaderLib.physical.uniforms.iblIrradianceDisabled =
    globalIblIrradianceDisabledUniform;
  const { clone } = UniformsUtils;
...
class CopyPass extends Pass {
  ...
  setTextureCount(textureCount) {
    ...
    (_this$fullscreenMater = this.fullscreenMaterial) == null
      ? void 0
      : _this$fullscreenMater.dispose();
...
class TemporalReprojectMaterial extends ShaderMaterial {
  constructor(textureCount = 1, customComposeShader = "") {
    let finalFragmentShader = fragmentShader$2.replace(
      "#include <reproject>",
      reproject
    );
    ...
    const matches = finalFragmentShader.matchAll(
      /inputTexture\[\s*[0-9]+\s*]/g
    );
    ...
    if (typeof customComposeShader === "string")
      this.defines.useCustomComposeShader = "";
  }
}
...
class TemporalReprojectPass extends Pass {
  constructor(
    ...
    if (options.neighborhoodClamping)
    ...
    this.fullscreenMaterial.uniforms.constantBlend.value =
      options.constantBlend;
    ...
    this.fullscreenMaterial.uniforms.prevProjectionMatrixInverse.value =
      camera.projectionMatrixInverse.clone(); // init copy pass to save the accumulated textures and the textures from the last frame
  ...
  render(renderer) {
    ...
    this.fullscreenMaterial.uniforms.lastDepthTexture.value =
      this.velocityDepthNormalPass.lastDepthTexture;
...
class TRAAEffect extends Effect {
  ...
  update(renderer, inputBuffer) {
    ...
    const noJitterMeshes = getVisibleChildren(this._scene).filter((c) =>
      isGroundProjectedEnv(c)
    );
    ...
    this.temporalReprojectPass.fullscreenMaterial.uniforms.inputTexture0.value =
      inputBuffer.texture;
...
class DenoisePass extends Pass {
  constructor(
    ...
    for (let i = 0; i < this.textures.length; i++) {
      this.fullscreenMaterial.uniforms["texture" + i] = new Uniform(
        textures[i]
      );
    }

    if (typeof options.roughnessDependent === "boolean") {
      options.roughnessDependent = Array(textures.length).fill(
        options.roughnessDependent
      );
    }
    ...
    if (typeof options.basicVariance === "number") {
      options.basicVariance = Array(textures.length).fill(
        options.basicVariance
      );
    }

    this.fullscreenMaterial.defines.basicVariance =
      /* glsl */
      `float[](${options.basicVariance
  ...
  keepEdgeStoppingDefinesUpdated() {
    for (const [name, phi, define] of useEdgeStoppingTypes) {
      ...
      const useEdgeStoppingType =
        this.options[name] &&
        (phi === "" ||
          ((_this$fullscreenMater = this.fullscreenMaterial.uniforms[phi]) ==
          null
            ? void 0
            : _this$fullscreenMater.value) > 0.001);
  ...
  render(renderer) {
    ...
    if (this.iterations > 0) {
      for (let i = 0; i < 2 * this.iterations; i++) {
        ...
        this.fullscreenMaterial.uniforms.isLastIteration.value =
          i === 2 * this.iterations - 1;
...
class SVGFTemporalReprojectPass extends TemporalReprojectPass {
  constructor(
    ...
    this.fullscreenMaterial.fragmentShader =
      momentBuffers + this.fullscreenMaterial.fragmentShader;
    ...
    this.copyPass.fullscreenMaterial.uniforms[
      "inputTexture" + (copyPassTextureCount - 1)
    ].value = this.momentTexture;
    const lastMomentTexture =
      this.copyPass.renderTarget.texture[copyPassTextureCount - 1];
    ...
    this.fullscreenMaterial.uniforms.lastMomentTexture.value =
      lastMomentTexture;
    this.fullscreenMaterial.defines.momentTextureCount = Math.min(
      2,
      textureCount
    );
  }
...
class SVGF {
  constructor(
    ...
    const textures = this.svgfTemporalReprojectPass.renderTarget.texture.slice(
      0,
      textureCount
    );
    ...
    this.denoisePass.fullscreenMaterial.uniforms.momentTexture.value =
      this.svgfTemporalReprojectPass.momentTexture;
  ...
  setNonJitteredDepthTexture(depthTexture) {
    this.svgfTemporalReprojectPass.fullscreenMaterial.uniforms.depthTexture.value =
      depthTexture;
  }

  setVelocityTexture(texture) {
    this.svgfTemporalReprojectPass.fullscreenMaterial.uniforms.velocityTexture.value =
      texture;
  }
...
class SSGIPass extends Pass {
  ...
  initMRTRenderTarget() {
    ...
    this.fullscreenMaterial.uniforms.emissiveTexture.value =
  ...
  setMRTMaterialInScene() {
    ...
    for (const c of this.visibleMeshes) {
      ...
      if (textureKey)
        mrtMaterial.uniforms.uvTransform.value =
          originalMaterial[textureKey].matrix;
      if (originalMaterial.emissive)
        mrtMaterial.uniforms.emissive.value = originalMaterial.emissive;
      if (originalMaterial.color)
        mrtMaterial.uniforms.color.value = originalMaterial.color; // update the child's MRT material
      ...
      mrtMaterial.uniforms.emissiveIntensity.value =
...
class SSGIEffect extends Effect {
  ...
  constructor(
    ...
    this.svgf.setJitteredGBuffers(
      this.ssgiPass.depthTexture,
      this.ssgiPass.normalTexture
    ...
    this.ssgiPass.fullscreenMaterial.defines.directLightMultiplier =
      options.directLightMultiplier.toPrecision(5);
      ...
      Object.defineProperty(this, key, {
        ...
        set(value) {
          ...
          switch (key) {
            ...
            case "normalPhi":
            case "roughnessPhi":
              this.svgf.denoisePass.fullscreenMaterial.uniforms[key].value =
                value;
              break;
            // SSGI
  ...
  update(renderer, inputBuffer) {
    ...
    globalIblIrradianceDisabledUniform.value =
      fullGi || this.diffuseOnly === true;
    globalIblRadianceDisabledUniform.value =
      fullGi || this.specularOnly == true;
...
class VelocityDepthNormalPass extends Pass {
  ...
  setVelocityDepthNormalMaterialInScene() {
    ...
    for (const c of this.visibleMeshes) {
      ...
      c.material = velocityDepthNormalMaterial;
      c.visible = isChildMaterialRenderable(c, originalMaterial);
      if (this.renderDepthNormal)
        velocityDepthNormalMaterial.defines.renderDepthNormal = "";
      const map =
  ...
  setSize(width, height) {
    ...
    this.renderTarget.setSize(width, height);
    (_this$lastDepthTextur = this.lastDepthTexture) == null
      ? void 0
      : _this$lastDepthTextur.dispose();
  ...
  get texture() {
    return Array.isArray(this.renderTarget.texture)
      ? this.renderTarget.texture[1]
