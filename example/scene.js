...
export const initScene = (name) => {
  ...
  const context = isWebGL2Available()
  ...
  loadPromises.push(
    new Promise((resolve) => {
      if (demo.lut) {
        new POSTPROCESSING.LUT3dlLoader().load(
          demo.modelName + ".3dl",
          (lutTexture) => {
  ...
  Promise.all(loadPromises).then(() => {
    ...
    const lensDistortionPass = new POSTPROCESSING.ShaderPass(
      lensDistortionMaterial
    );
    ...
    Object.defineProperty(lensDistortionMaterial, "useSrgbEncoding", {
      get: () =>
        composer.passes.indexOf(lensDistortionPass) ===
        composer.passes.length - 1,
    });

    if (demo.settings.compressionPass === true)
      compressionPass = new CompressionPass();
    ...
    composer.addPass(
      new EffectPass(camera, traaEffect, ...mainEffects, ...effects, lutEffect)
    );
    ...
    if (demo.settings.compressionPass === true)
      compressionPass.addToComposer(composer, secondLast);
  });
  ...
  new RGBELoader()
    ...
    .load("kloofendal_48d_partly_cloudy_1k.hdr", (tex) => {
  ...
  detectSupport().then((result) => {
    ...
    gltflLoader.load(
      demo.modelName + ".optimized.glb",
      setupWorldMesh,
      (ev) => {
        const progress = Math.round((ev.loaded / demo.size) * 100);
        document.querySelector("#loading").textContent =
          progress === 100 ? "Initialising..." : progress + "%";
      }
    );
  });
  ...
  function setupWorldMesh(asset) {
    ...
    if (demo.collisions) {
      ...
      worldOctree.fromGraphNode(
        new THREE.Mesh(
          new THREE.PlaneBufferGeometry(128, 128).rotateX(-Math.PI / 2)
        )
      );
    }
    ...
    asset.scene.traverse((c) => {
      if (c.isMesh) {
        ...
        if (c.material.roughnessMap)
          c.material.roughnessMap.anisotropy = anisotropy;
        if (c.material.metalnessMap)
          c.material.metalnessMap.anisotropy = anisotropy;
        ...
        if (c.name === demo.reflectiveFloorName) {
          ...
          let meshReflectorMaterial = new MeshReflectorMaterial(
            renderer,
            camera,
            scene,
            c,
          ...
          meshReflectorMaterial.onBeforeRenderReflections = () => {
            if (skyMesh) skyMesh.material.color.setScalar(1.675);
            ...
            for (let mesh of sceneMeshes) {
              if (mesh.visible && mesh.userData.lowResMaterial)
                mesh.material = mesh.userData.lowResMaterial;
            }
          };
        ...
        if (!(c.material instanceof MeshReflectorMaterial))
          c.enhanceShaderLightingMaterial = c.material;
  ...
  function syncGui() {
    ...
    hueSaturationEffect.uniforms.get("saturation").value =
      generalParams.saturation;
    ...
    lensDistortionMaterial.uniforms.jitterIntensity.value =
      viewParams.jitterIntensity;
    ...
    bloom1Effect.luminanceMaterial.threshold =
      bloomParams.bloom1_luminanceThreshold;
    bloom1Effect.luminanceMaterial.smoothing =
      bloomParams.bloom1_luminanceSmoothing;
    ...
    bloom2Effect.luminanceMaterial.threshold =
      bloomParams.bloom2_luminanceThreshold;
    bloom2Effect.luminanceMaterial.smoothing =
      bloomParams.bloom2_luminanceSmoothing;
  }
  ...
  function loadLightmaps(type, folder) {
    ...
    const iterate = [
      ...
      [
        materials.find((mat) => mat.name === "trackcollideable"),
        "track.collideable_denoised",
      ],
    ];
  ...
  const createGui = () => {
    ...
    generalParams = {
      ...
      "clipboard settings"() {
        ...
        for (let prop in generalParams) {
          if (
            prop === "enhanceShaderLighting" ||
            typeof generalParams[prop] === "function"
          )
            continue;
    ...
    generalFolder
      .add(generalParams, "enhanceShaderLighting")
      .onChange((val) => {
        if (val) {
          scene.traverse((c) => {
    ...
    if (demo.settings["color lut"] === true)
      generalFolder.add(generalParams, "color lut").onChange((val) => {
        lutEffect.blendMode.setBlendFunction(
          val
            ? POSTPROCESSING.BlendFunction.SRC
            : POSTPROCESSING.BlendFunction.SKIP
        );
      });
    ...
    generalFolder
      .add(generalParams, "fogDensity")
      .step(0.0001)
      .min(0)
      .max(0.01);
    generalFolder
      .add(generalParams, "toneMappingExposure")
      .min(0)
      .max(5)
      .step(0.0125);
    generalFolder
    ...
    generalFolder.add(generalParams, "gamma").step(0.0125).min(0).max(1.75);
    generalFolder
      .add(generalParams, "saturation")
      .step(0.005)
      .min(-0.5)
      .max(0.5);

    generalFolder
      .add(generalParams, "envMapIntensity")
      .step(0.01)
      .min(0)
      .max(25);
    generalFolder
      .add(generalParams, "lightMapIntensity")
      .step(0.01)
      .min(0)
      .max(1);
    ...
    lightMapFolder
      .add(generalParams, "loadWebPLightmaps")
      .name("WebP (456 KB)");
    lightMapFolder
      .add(generalParams, "loadAvifLightmaps")
      .name("AVIF (368 KB)");
    lightMapFolder
      .add(generalParams, "envMapPosX")
      .step(0.01)
      .min(-100)
      .max(100);
    lightMapFolder
      .add(generalParams, "envMapPosY")
      .step(0.01)
      .min(-100)
      .max(100);
    lightMapFolder
      .add(generalParams, "envMapPosZ")
      .step(0.01)
      .min(-100)
      .max(100);
    ...
    if (demo.settings.aoColor)
      aoColor.setHex(demo.settings.aoColor).getHSL(aoColorHsl);
    ...
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "aoPower")
      .step(0.05)
      .min(0)
      .max(12);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "aoSmoothing")
      .step(0.01)
      .min(0)
      .max(1);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "aoMapGamma")
      .step(0.01)
      .min(0.5)
      .max(1.5);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "lightMapGamma")
      .step(0.01)
      .min(0.5)
      .max(1.5);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "lightMapSaturation")
      .step(0.01)
      .min(0)
      .max(2.5);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "envPower")
      .step(0.05)
      .min(0)
      .max(16);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "smoothingPower")
      .step(0.01)
      .min(0)
      .max(1);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "roughnessPower")
      .step(0.05)
      .min(0)
      .max(4);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "irradianceIntensity")
      .step(0.01)
      .min(0)
      .max(10);
    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "radianceIntensity")
      .step(0.01)
      .min(0)
      .max(10);

    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "sunIntensity")
      .step(0.01)
      .min(0)
      .max(15);

    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "mapContrast")
      .step(0.005)
      .min(0.5)
      .max(1.5);

    enhanceShaderLightingFolder
      .add(enhanceShaderLightingParams, "lightMapContrast")
      .step(0.01)
      .min(0.5)
      .max(1.5);
    ...
    // }).listen()
    enhanceShaderLightingFolder.addColor(
      enhanceShaderLightingParams,
      "hemisphereColor"
    );
    enhanceShaderLightingFolder.addColor(
    ...
    bloomFolder
      .add(bloomParams, "bloom1_luminanceThreshold")
      .step(0.01)
      .min(0)
      .max(2);
    bloomFolder
      .add(bloomParams, "bloom1_luminanceSmoothing")
      .step(0.01)
      .min(0)
      .max(2);
    ...
    bloomFolder
      .add(bloomParams, "bloom2_luminanceThreshold")
      .step(0.01)
      .min(0)
      .max(2);
    bloomFolder
      .add(bloomParams, "bloom2_luminanceSmoothing")
      .step(0.01)
      .min(0)
      .max(2);
    ...
    if (scene.getObjectByName(demo.reflectiveFloorName)) {
      const meshReflectorMaterial = scene.getObjectByName(
        demo.reflectiveFloorName
      ).material;
      if (meshReflectorMaterial instanceof MeshReflectorMaterial) {
        ...
        meshReflectorMaterial.kawaseBlurPass.kernelSize =
          reflectionOptions.blurKernelSize;
        ...
        reflectiveFloorFolder
          .add(reflectionOptions, "useReflector")
          .onChange((val) => {
            const floor = scene.getObjectByName(demo.reflectiveFloorName);
            floor.material = val
              ? floor.userData.meshReflectorMaterial
              : floor.userData.meshStandardMaterial;
          });
        ...
        reflectiveFloorFolder
          ...
          .onChange((val) => {
            ...
            meshReflectorMaterial.fbo1.depthTexture = new DepthTexture(
              val,
              val
            );
        ...
        reflectiveFloorFolder
          .add(props, "mixStrength")
          .step(0.01)
          .min(0)
          .max(30);
        reflectiveFloorFolder
          .add(props, "minDepthThreshold")
          .step(0.01)
          .min(0.5)
          .max(1.5);
        reflectiveFloorFolder
          .add(props, "maxDepthThreshold")
          .step(0.01)
          .min(0.5)
          .max(1000);
        reflectiveFloorFolder.add(props, "depthScale").step(0.01).min(0).max(1);
        reflectiveFloorFolder
          .add(props, "depthToBlurRatioBias")
          .step(0.01)
          .min(0)
          .max(1);
        reflectiveFloorFolder.add(props, "distortion").step(0.01).min(0).max(5);
        reflectiveFloorFolder
          .add(props, "mixContrast")
          .step(0.01)
          .min(0)
          .max(2);
        reflectiveFloorFolder
          .add(props, "roughnessAdd")
          .step(0.001)
          .min(0)
          .max(0.25);
        reflectiveFloorFolder
          .add(props, "reflectionPower")
          .step(0.1)
          .min(0)
          .max(16);
        reflectiveFloorFolder
          .add(props, "envMapMixStrength")
          .step(0.1)
          .min(0)
          .max(5);
        reflectiveFloorFolder
          .add(props, "reflectionSaturation")
          .step(0.01)
          .min(0)
          .max(1);
      }
    ...
    configureMaterialParams = {
      ...
      "clipboard values"() {
        ...
        const colorHex =
          colorScalar === 0 ? "0x" + material["color"].getHexString() : "";
        ...
        const output = `
          ...
          colorScalar === 0 ? "" : ".setScalar(" + colorScalar + ")"
        },
                ${normalScale},
                userData: { noValueOverride: true }
            })
    ...
    configureMaterialFolder
      .addColor(configureMaterialParams, "color")
      .onChange((val) => {
        pickedMesh.material["color"]
          .setHex(val)
          .multiplyScalar(configureMaterialParams["colorScalar"]);
      });
  ...
  document.addEventListener("mousedown", (ev) => {
    ...
    canvas.style.filter =
      canvas.style.filter === "blur(12px)" ? "none" : "blur(12px)";
  });
