...
return env;
`;
const map_fragment = THREE__namespace.ShaderChunk.map_fragment.replace("diffuseColor *= sampledDiffuseColor;", /* glsl */`
    #ifdef USE_LIGHTMAP
    ...
    `);
const lights_pars_begin = THREE__namespace.ShaderChunk.lights_pars_begin.replace("vec3 irradiance = ambientLightColor;", /* glsl */`
    ${aoCode}
    aoMapClr *= aoMapClr;
...
const lights_fragment_maps = THREE__namespace.ShaderChunk.lights_fragment_maps.replace("USE_LIGHTMAP", "false");
const lightmap_fragment = THREE__namespace.ShaderChunk.lightmap_fragment.replace("USE_LIGHTMAP", "false");
...
function enhanceShaderLighting(shader, {
  aoColor = new THREE__namespace.Color(0x000000),
  hemisphereColor = new THREE__namespace.Color(0xffffff),
  irradianceColor = new THREE__namespace.Color(0xffffff),
  radianceColor = new THREE__namespace.Color(0xffffff),
  aoPower = 1,
  aoSmoothing = 0,
  aoMapGamma = 1,
  lightMapGamma = 1,
  ...
  if (hardcodeValues) {
    shader.fragmentShader = shader.fragmentShader.replace("uniform float opacity;", /* glsl */`
  ...
  `).replace("main() {", `
      main() {
      ${aoCode}
      `).replace("#include <aomap_fragment>", "").replace("#include <lights_pars_begin>", lights_pars_begin).replace("#include <lights_fragment_maps>", lights_fragment_maps).replace("#include <lightmap_fragment>", lightmap_fragment).replace("#include <map_fragment>", map_fragment).replace("#include <envmap_physical_pars_fragment>", THREE__namespace.ShaderChunk.envmap_physical_pars_fragment).replace("getIBLIrradiance( const in vec3 normal )", "getIBLIrradiance( const in vec3 normal, float aoMapClr )").replace("getIBLRadiance( const in vec3 viewDir, const in vec3 normal, const in float roughness )", "getIBLRadiance( const in vec3 viewDir, const in vec3 normal, const in float roughness, float aoMapClr )").replace("return PI * envMapColor.rgb * envMapIntensity;", getIBLIrradiance_replace).replace("return envMapColor.rgb * envMapIntensity;", getIBLRadiance_replace).replace(" #if defined( USE_ENVMAP ) && defined( RE_IndirectSpecular )", /* glsl */`
            #include <aomap_fragment>
            #if defined( USE_ENVMAP ) && defined( RE_IndirectSpecular )
            `).replace("iblIrradiance += getIBLIrradiance( geometry.normal );", /* glsl */`
            iblIrradiance += getIBLIrradiance( geometry.normal, aoMapClr );
            `).replace("radiance += getIBLRadiance( geometry.viewDir, geometry.normal, material.roughness );", /* glsl */`
            radiance += getIBLRadiance( geometry.viewDir, geometry.normal, pow(material.roughness, roughnessPower), aoMapClr );
            `).replace("clearcoatRadiance += getIBLRadiance( geometry.viewDir, geometry.clearcoatNormal, material.clearcoatRoughness );", /* glsl */`
...
exports.enhanceShaderLighting = enhanceShaderLighting;
