...
function updatePlayer(deltaTime) {
    ...
    if (flyMode) {
        ...
        if ((keyStates["keyW"] || keyStates["keyS"] || keyStates["keyA"] || keyStates["keyD"]) && (keyStates["keyA"] || keyStates["keyD"])) {
            speed *= 0.5 ** 0.5;
        }
        ...
        weight = (playerVelocity.x ** 2 + playerVelocity.z ** 2) ** 0.5;
        weight *= 0.00225 / speedMultiplier;
        if (weight > 0.1) weight = 0.1;
    }

    let weightDelta = Math.max((flyMode ? 0.75 : 15) * deltaTime, 1);

    camera.clip.weight = THREE.MathUtils.lerp(camera.clip.weight, 1.5 * weight, weightDelta);
    camera.clip.timeScale = THREE.MathUtils.lerp(camera.clip.timeScale, 3.2 * (0.6 + weight * 4), weightDelta);

    camera.rotation.x -= 0.06 * camera.clip.weight;
    camera.rotation.y -= 0.06 * camera.clip.weight;
