...
import {
  DepthFormat, DepthTexture, LinearFilter, Matrix4, PerspectiveCamera, Plane, UnsignedShortType, Vector3, Vector4, WebGLRenderTarget,
} from "three";
...
class MeshReflectorMaterial extends THREE.MeshStandardMaterial {
  ...
  setupBuffers(resolution, blur, bufferSamples) {
    ...
    this.kawaseBlurPass = new POSTPROCESSING.KawaseBlurPass({ width: blur[0], height: blur[1], resolutionX: blur[0], resolutionY: blur[1] });
    this.kawaseBlurPass.kernelSize = POSTPROCESSING.KernelSize.HUGE;
  ...
  onBeforeCompile(shader, ...args) {
    ...
    if (shader.defines && shader.defines.ENHANCE_SHADER_LIGHTING !== undefined) {
